...
st.markdown("""
    ...
    /* 5. Kartu Utama (Size Chart & Laporan) */
    .solid-card {
    ...
    /* 6. DESAIN KARTU INVENTORY (YANG LU MAU DIPISAH) */
    .inventory-card {
        background: #181818 !important; /* Warna kartu sedikit lebih terang dari background */
        border: 1px solid #444 !important; /* Border tegas biar kelihatan misah */
        border-radius: 12px; 
        padding: 20px; 
        margin-bottom: 30px !important; /* Jarak antar kartu lu lebarin di sini */
        display: flex; 
        justify-content: space-between; 
        align-items: center; 
        box-shadow: 0 10px 20px rgba(0,0,0,0.5); /* Efek bayangan biar 'ngambang' */
    }
...
if st.button("Calculate", use_container_width=True):
    ...
    pred_unit = model.predict([[tinggi, berat, ml_kat, 0, cut_val]])[0]
    ...
    with res_col2:
        ...
        st.markdown(f'<div class="solid-card" style="padding:0; overflow:hidden;"><div style="padding:20px;"><h4 class="card-header" style="border:none; margin:0;">Size Chart ({cutting})</h4></div><div style="overflow-x:auto;">{table_html}</div></div>', unsafe_allow_html=True)

    # -- PROCUREMENT CARDS (THE FIX) --
    kain_cards = ""
